...
static bool s_sampleServicesStarted = false;
...
static bool DjiUser_IsDirectionControlDemoRequested(int argc, char **argv);
...
int main(int argc, char **argv)
{
    ...
    bool directionControlDemoRequested = DjiUser_IsDirectionControlDemoRequested(argc, argv);
    ...
    if (!directionControlDemoRequested) {
        s_sampleServicesStarted = true;
    ...
    if (directionControlDemoRequested) {
        ...
        returnCode = DjiTest_FlightControlRunSample(
            E_DJI_TEST_FLIGHT_CTRL_SAMPLE_SELECT_MANUAL_DIRECTION_CTRL);
        cleanupReturnCode = DjiUser_CleanSystemEnvironment();
        if (cleanupReturnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Clean system environment after direction demo failed, error code:0x%08llX",
                           cleanupReturnCode);
            return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
        }
...
static bool DjiUser_IsDirectionControlDemoRequested(int argc, char **argv)
{
    ...
    for (argumentIndex = 1; argumentIndex < argc; ++argumentIndex) {
        if (strcmp(argv[argumentIndex], "--direction-demo") == 0) {
            return true;
        }
    }
